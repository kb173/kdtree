...
kdtree::search_rec(kdtree::node *root, kdtree::rect *current_bounds, kdtree::point_heap *best, kdtree::circ *c) {
    ...
    if (best->add(root->p, dist)) { // Try inserting point into heap
    ...
    bool intersect_left = true;
    bool intersect_right = true;

    // TODO: Set intersect based on whether bound rect and circle overlap

    // Search a path if there is a next node at that path AND the searchpoint
            // is either on that side OR the bounds overlap
    if (root->left && (is_left || intersect_left)) {
        cout << "Continuing left" << endl;
        best = search_rec(root->left, current_bounds, best, c);
    }

    if (root->right && (!is_left || intersect_right)) {
        cout << "Continuing right" << endl;
        best = search_rec(root->right, current_bounds, best, c);
    }

    return best; // Exit condition: No node was either on the right side or intersected
}
